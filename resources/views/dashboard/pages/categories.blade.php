          ...
          <div class="box-body table-responsive no-padding">
            <table class="table table-hover">
              ...
              <tr>
                ...
                <td><a href="{{  asset('Categories_Image/' . $Category['photo']) }}">Show Photo</a>
                </td>
                <td>
                       <!-- Start Form Edit Category -->
                    <button type="button" class="btn-sm btn-success" data-toggle="modal" data-target="#EditCategory{{$Category['id']}}">Edit</button>

                      <div class="modal fade bd-example-modal-sm" id="EditCategory{{$Category['id']}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add New Category</h5>
                          ...
                          <div class="modal-body">
                            <form method="POST" action="{{ route('categories.update',['categories'=> $Category['id']]) }}"  enctype="multipart/form-data" >
                               @csrf
                               @method('PUT')
                              <div class="form-group">
                                <label for="recipient-name" class="col-form-label">Category Name:</label>
                                <input type="text" class="form-control" id="recipient-name" value="{{$Category['name']}}" name="name"> 
                                       @error('name')
                                             <label class="text-danger"> {{ $message }}</label>
                                        @enderror
                              </div>
                              <div class="form-group">
                                ...
                                <div class="custom-file">
                                   <img src="{{  asset('Categories_Image/' . $Category['photo']) }}" class="img-thumbnail" style="max-height: 13rem; width: 100%; min-height: 13rem; ">
                                   <label class="custom-file-label" for="inputGroupFile01">Choose Image</label>
                                    <input type="file" class="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" name="image">
                                        @error('image')
                                             <label class="text-danger"> {{ $message }}</label>
